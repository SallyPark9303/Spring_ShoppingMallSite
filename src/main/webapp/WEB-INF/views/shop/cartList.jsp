...
.listResult .orderinfoinsert { float:right; width:45%; text-align:right; }
.listResult .orderinfoinsert button { font-size:18px; padding:5px 10px; border:1px solid #999; background:#fff;}
...
<body>
	<div id="root">
		...
		<section id="container">
			...
			}); //모두 선택 체크박스에 체크 선택 ->개별 체크박스 들도 선택됨.
			</script>
			...
			});  //confirm을 이용해 삭제 여부확인, 개별 선택된 체크박스들을 배열 변수 checkArr에 저장한 뒤 컨트롤러로 전송
			     // 전송에 이상이 없다면 현재 페이지 (/shop/cartList)를 새로고침.
    ...
    $(".delete${cartList.cartNum}Btn").click(function(){
		...
		<div class="orderinfoinsert">
		<button type="button" class="orderinfoBtn">주문 정보 입력</button>
		<script>
		$(".orderinfoBtn").click(function(){
		$(".orderInfo").slideDown();
		$(".orderinfoBtn").slideUp();
		});
	...
	<!-- name속성값이 있어야 폼 으로 전송할 때 원하는 형태의 데이터 타입으로 전송됨. -->
		<div class="inputArea">
