	...
	<form role="form" method="post" autocomplete="off" enctype="multipart/form-data">
		 ...
		 <select class="category1">
 		 <option value="">전체</option>
			...
			<textarea rows="5" cols="50" type="text" id="gdsDes" name="gdsDes"></textarea>
			<script>
			 		var ckeditor_config = {
			   resize_enaleb : false,
			 ...
			 CKEDITOR.replace("gdsDes", ckeditor_config); //텍스트에리어를 ckeditor로 교체
			</script>
			...
			<script> //파일이 등록되면 현재화면에서 선택된 이미지를 볼수 있게 해주는 스크립트.
...
for(var i = 0; i < jsonData.length; i++) {
 ...
 if(jsonData[i].level == "1") { //레벨이 1인 데이터가 있다면
  ...
  // cate1Obj에 cateCode와 cateName을저장
...
/*	 for(var i = 0; i < cate2Arr.length; i++) {
	 ...
	 $("option:selected", this).each(function(){
		 ...
		 var selectVal =$(this).val(); //selectVal은 현재(selected)된1차분류값을 저장
...
$("#gdsPrice").keyup(function(){numCheck($(this)); }); //gdsPrice, gdsStock를 입력할때마다 numCheck 함수 호출 이때 현재 선택자 $(this) 도 함께 보냄. 이때 $(this)는 상품 가격 일때는  $("#gdsPrice"), 상품 수량일때는 $("#gdsStock")
